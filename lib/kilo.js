...
db.transaction(
 ...
 $.get('http://ec2-54-245-31-205.us-west-2.compute.amazonaws.com/kilolungo/data/level.php',initialiseDatabase);
...
/*TO DO: this is crap but jsut to test */
	...
	$(element).replaceWith($('<img />').attr('src',row.image_url).bind("load",allLoaded));
...
function initialiseDatabase(data) {
	if ( typeof localStorage.version == 'undefined' || parseInt(localStorage.version) < parseInt($(data).find('levels').attr('version')) ) {
		db.transaction(
         function(transaction) {
           transaction.executeSql(
                'delete from entries;',
		...
		localStorage.maxId = $(data).find('word').length;
		localStorage.version = $(data).find('levels').attr('version');
		}
